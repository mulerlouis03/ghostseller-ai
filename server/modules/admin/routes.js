...
adminRouter.get("/stats",requireAuth,requireAdmin,async(req,res)=>{
 ...
 res.json({users:users.count||0,projects:projects.count||0,posts:posts.count||0,leads:leads.count||0,trends:trends.count||0,videos:videos.count||0});
});
adminRouter.get("/users",requireAuth,requireAdmin,async(req,res)=>{
 const {data,error}=await supabase.from("users").select("id,name,email,plan,credits,role,created_at").order("created_at",{ascending:false}).limit(100);
