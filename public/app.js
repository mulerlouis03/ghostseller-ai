...
async function generateAIContent(){
  ...
  const out=document.getElementById("contentResult");
  btn.disabled=true;
  btn.innerText="Génération IA...";
  out.innerHTML="<div class='item'>Génération en cours...</div>";
  ...
  try{
    ...
    out.innerHTML = `
      <div class="item">
        <h3>Hooks</h3>
        <pre>${esc(JSON.stringify(data.hooks || [], null, 2))}</pre>
      </div>
      <div class="item">
        <h3>Captions</h3>
        <pre>${esc(JSON.stringify(data.captions || [], null, 2))}</pre>
      </div>
      <div class="item">
        <h3>Hashtags</h3>
        <pre>${esc((data.hashtags || []).join(" "))}</pre>
      </div>
      <div class="item">
        <h3>Scripts</h3>
        <pre>${esc(JSON.stringify(data.scripts || [], null, 2))}</pre>
      </div>
      <div class="item">
        <h3>Post Instagram</h3>
        <pre>${esc(data.instagram_post || "")}</pre>
      </div>
      <div class="item">
        <h3>Message WhatsApp</h3>
        <pre>${esc(data.whatsapp_message || "")}</pre>
      </div>
    `;
  }catch(e){
    out.innerHTML="<div class='item'>Erreur génération IA.</div>";
  }finally{
    btn.disabled=false;
